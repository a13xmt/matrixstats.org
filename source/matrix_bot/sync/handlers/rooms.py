...
def room_message_handler(self, event):
    msg_type = event.get("content", {}).get("msgtype")
    msg_handler = message_event_handlers.get(msg_type)
    result = msg_handler(self, event) if msg_handler else None

def room_message_feedback_handler(self, event):
    pass

def room_name_handler(self, event):
    pass

def room_topic_handler(self, event):
    pass

def room_avatar_handler(self, event):
    pass

def room_pinned_events_handler(self, event):
    pass
